...
$sql = "SELECT Nombre, Link_Iframe, Ending FROM ed WHERE Link_Iframe != '' ORDER BY `ed`.`ID` DESC";
...
if ($result->num_rows > 0) {
  while ($row = $result->fetch_assoc()) {
    $enlaces_iframe[$row['Nombre']] = ['Link' => $row['Link_Iframe'], 'Ending' => $row['Ending']];
  }
}
    ...
    <div class="form-group">
      ...
      <datalist id="open">
        ...
        foreach ($enlaces_iframe as $nombre => $info) {
          echo "<option value='" . $nombre . " ED " . $info['Ending'] . " - " . $info['Link'] . " '></option>";
        }
